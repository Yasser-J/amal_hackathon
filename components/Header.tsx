...
const Header: React.FC<HeaderProps> = ({ 
  ...
  return (
    <header className="h-16 flex items-center justify-between px-8 bg-[var(--edix-surface-light-card)] dark:bg-[var(--edix-surface-dark-1)] border-b border-gray-100 dark:border-slate-800 sticky top-0 z-40 transition-all">
      <div className="flex items-center gap-10">
        <div className="flex items-center">
          <div className="hidden md:block">
            ...
            <p className="text-[10px] text-[#004A78] dark:text-[var(--edix-primary-soft)] font-bold uppercase tracking-[0.15em] mt-1">{t.assistant}</p>
          </div>
        </div>

        {/* Navigation Tabs - Moved to Top Nav Bar */}
        <nav className="flex items-center bg-slate-50/60 dark:bg-[var(--edix-surface-dark-2)] p-1 rounded-xl border border-gray-100 dark:border-slate-700">
          <button 
            ...
            className={`flex items-center gap-2 px-6 py-1.5 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${
              currentView === 'dashboard' || currentView === 'review'
                ? 'bg-white dark:bg-[var(--edix-surface-dark-3)] text-[var(--edix-primary-soft)] shadow-sm' 
      ...
      <div className="flex items-center gap-3">
        <div className="relative">
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-xl border border-gray-100 dark:border-slate-800 text-[11px] font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-[var(--edix-surface-dark-2)] transition-all"
          >
            <Globe size={14} className="text-[#00629D]" />
            ...
            <div className={`absolute ${language === 'ar' ? 'left-0' : 'right-0'} mt-2 w-32 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-gray-100 dark:border-slate-800 overflow-hidden z-50 animate-universal`}>
              <div className="p-1.5 space-y-1">
                <button 
                  ...
                  className={`w-full flex items-center justify-between px-4 py-2.5 text-xs font-bold rounded-xl transition-all ${language === 'en' ? 'bg-[var(--edix-primary-surface)] text-[#004A78] dark:bg-[var(--edix-surface-dark-2)]' : 'text-slate-600 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-800'}`}
                >
                  <span>ENGLISH</span>
                ...
                <button 
                  ...
                  className={`w-full flex items-center justify-between px-4 py-2.5 text-xs font-bold rounded-xl transition-all ${language === 'ar' ? 'bg-[var(--edix-primary-surface)] text-[#004A78] dark:bg-[var(--edix-surface-dark-2)]' : 'text-slate-600 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-800'}`}
                >
                  <span>العربية</span>
